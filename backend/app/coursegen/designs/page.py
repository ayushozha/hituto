"""Page design agent (specs/design_agents §3): the HTML capsule author.
...
1. HTML section fan-out (fast_gen §4.2) — parallel per-section HTML streamed into the
   shell iframe via ``gen_fragment`` (SVG/canvas-first). This is the default UX:
   GenerationTheater morphs live while the chapter generates.
2. Whole-document HTML — streaming Mode B; repair attempts optionally hedge a
   repair-prompted generation against a fresh rewrite (fast_gen §2.4).
3. A2UI (opt-in ``COURSEGEN_A2UI_LESSONS`` only) — last resort when HTML paths fail.
   Kept for experiments; not the product default.
"""
...
class PageAgent:
    ...
    async def author(self, ctx: AuthorContext) -> DesignOutput:
        ...
        # 1) HTML section fan-out — live gen_fragment streaming (product default).
        ...
        # 2) Whole-document HTML (Mode B stream / hedged repair).
        ...
        return DesignOutput(kind="html", html=html or "", plan=plan)
